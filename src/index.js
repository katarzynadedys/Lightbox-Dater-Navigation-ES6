class Lightbox {
  ...
  addListenerToImages() {
    for (let i = 0; i < this.allImages.length; i++) {
      this.allImages[i].addEventListener("click", () => {
        this.changeLightboxImgSrc(this.allImages[i]);
      });
    }
  }
...
class Navigation {
  ...
  addListenerToMenu() {
    ...
    for (let i = 0; i < this.navLinks.length; i++) {
      this.navLinks[i].addEventListener("click", (evt) =>
        this.manipulateDropdown(evt)
      );
    }
  }
